...
1. It handles multiple conflicting objectives (data fit, smoothness, physical constraints)
...
[2] D. Price, M.I. Radaideh, "Animorphic Ensemble Optimization: a large-scale island model",
    ...
    **kwargs
    ...
    for cycle in range(n_cycles):
        ...
        if not is_convergence_phase and len(island_hvs) > 1:
            ...
            if verbose:
                print(f"  Best island: {best_island_name} (HV={island_hvs[best_island_name]:.4f})")
    ...
    if len(all_pareto_objectives) > 0:
        ...
        warnings.warn("MAEO did not find valid solutions. Using least-squares fallback.")
    ...
    result = {
        ...
        "best_algorithm": algorithms[best_island_idx] if 'best_island_idx' in locals() else algorithms[0],
        "hypervolume_history": hv_history,
        "population_history": pop_history,
        "pareto_front": all_pareto_objectives if len(all_pareto_objectives) > 0 else None,
