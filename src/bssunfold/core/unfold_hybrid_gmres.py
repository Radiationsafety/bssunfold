...
def unfold_hybrid_gmres(
    ...
    if beta < 1e-14:
        ...
        return {
            ...
            "effective_readings": {name: float(val) for name, val in zip(selected, computed_readings)},
    ...
    H = np.zeros((max_krylov + 1, max_krylov))
    V[:, 0] = v1
    ...
    for k in range(max_krylov):
        ...
        if regularization_method == "gcv" or regularization_method == "modgcv":
            ...
            if len(gcv_values) >= 3:
                ...
                if best_gcv_val > recent_min * 1.01:  # GCV minimum increased
                    ...
                    logger.info(f"GCV minimum increased at iteration {current_k}")
    ...
    if best_solution is None:
        spectrum = np.maximum(x0 + U[:, :actual_k] @ y_lambda, 0) if 'y_lambda' in locals() else np.maximum(x0, 0)
    else:
