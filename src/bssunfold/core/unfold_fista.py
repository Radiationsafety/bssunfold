...
def _project_box(x: np.ndarray, x_min: float = 0.0, x_max: float = np.inf) -> np.ndarray:
...
def unfold_fista(
    ...
    for k in range(max_iterations):
        ...
        # Check convergence
        rel_change = np.linalg.norm(x - x_old) / max(np.linalg.norm(x_old), 1e-10)
        if rel_change < tolerance:
            logger.info(f"FISTA converged at iteration {k+1}")
            break
        
        # Discrepancy principle stopping
        if discrepancy_threshold is not None and current_residual <= discrepancy_threshold:
    ...
    result = {
        ...
        "doserates": doserates,
        "iterations": k + 1,
        "final_residual_norm": float(residuals[-1]) if residuals else float(np.linalg.norm(residual)),
