...
- **unfold_mcmc's lazy loader never evicts `sys.modules`** (`_load_pymc`). The `block_import` mock patches `builtins.__import__`, so it intercepts even cached `pymc`/`arviz` imports — a purge is unnecessary and was actively harmful: blocked-import fixtures (test_boost_part2.py) left `sys.modules['pymc']` evicted while submodules stayed cached, and the next real re-import built a cross-bound module whose internal `pm.NUTS` lookups crashed `pm.sample` on macOS with pymc >= 6 (uv.lock resolves pymc 6.3.2 on Python >= 3.12). Do not re-add `sys.modules.pop` to `_load_pymc`. `_run_nuts_pymc` additionally retries with an explicit `step=pymc.step_methods.NUTS(...)` if PyMC raises the `'NUTS'` AttributeError.
