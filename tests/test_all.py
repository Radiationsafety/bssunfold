...
class TestDetector:
    ...
    def test_effective_readings(self, detector):
        ...
        spectrum_df = pd.DataFrame({"E_MeV": detector.E_MeV, "Phi": spectrum})
    ...
    def test_discretize_spectra(self, detector):
        ...
        spectrum_dict = {"E_MeV": E_coarse, "Phi": np.ones(20)}
...
class TestDirectFunctions:
    ...
    def test_solve_kaczmarz(self):
        ...
        x_opt, n_iter, converged = solve_kaczmarz(A, b, x0, max_iterations=500)
        assert x_opt.shape == (10,)
        assert np.all(x_opt >= 0)
