...
class TestInterpretPyopt:
    ...
    def test_build_interpretation_qp_validation_bad_shape(self):
        ...
        saved = _pyopt._loaded
        try:
            # pyoptexplain is required for the validation, which runs after the
            # import check. Since pyoptexplain isn't installed, the import fires first.
...
class TestUnfoldSMT:
    ...
    def test_build_constraints_row_len_mismatch_raises(self):
        ...
        with pytest.raises(ValueError, match="ill-formed"):
            _build_constraints(
                xs, [[1, 2, 3]], [1], mock_z3, lambda v, z: z.IntVal(int(v))
            )
...
class TestUnfoldLmfit:
    ...
    def test_residual_elastic_leastsq(self):
        ...
        A = np.eye(3)
        b = np.array([1.0, 2.0, 3.0])
        result = _residual_elastic(params, A, b, 0.01, 0.01, 0.5, "leastsq", 3)
    ...
    def test_residual_elastic_scalar(self):
        ...
        A = np.eye(3)
        b = np.array([1.0, 2.0, 3.0])
        result = _residual_elastic(params, A, b, 0.01, 0.01, 0.5, "nelder", 3)
        assert isinstance(result, float)
...
class TestUnfoldDocplex:
    ...
    def test_import_docplex_cplex_blocked(self):
        """_import_docplex raises ImportError when cplex is blocked."""
...
class TestUnfoldMCMC:
    ...
    def test_getattr_unknown(self):
        ...
        with pytest.raises(AttributeError, match="has no attribute"):
            _ = mcmc_mod.nonexistent_attribute_xyz
