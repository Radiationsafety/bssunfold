...
class TestSolveMlemStop:
    ...
    def test_basic(self):
        ...
        sol, iters, conv = solve_mlem_stop(
            A, b, x0, max_iterations=200, cps_crossover=100.0
        )
    ...
    def test_with_explicit_threshold(self):
        ...
        sol, iters, conv = solve_mlem_stop(
            A, b, x0, max_iterations=5000, j_threshold=100.0
        )
        assert conv
        assert iters == 1
    ...
    def test_zero_b(self):
        ...
        sol, iters, conv = solve_mlem_stop(
            A, b, x0, max_iterations=10, cps_crossover=1.0
        )
        assert sol.shape == (n_bins,)
        assert iters == 10
        assert not conv
    ...
    def test_zero_threshold(self):
        ...
        sol, iters, conv = solve_mlem_stop(
            A, b, x0, max_iterations=5, j_threshold=0.0
        )
        assert iters == 1
        assert conv
    ...
    def test_all_same_readings(self):
        ...
        sol, iters, conv = solve_mlem_stop(
            A, b, x0, max_iterations=200, cps_crossover=10.0
        )
        assert sol.shape == (n_bins,)
        assert np.all(sol >= 0)
    ...
    def test_convergence_behaviour(self):
        ...
        sol, iters, conv = solve_mlem_stop(
            A, b, x0, max_iterations=5000, cps_crossover=10.0
        )
        assert iters > 0
...
class TestUnfoldMlemStop:
    ...
    def test_basic_unfolding(self, detector, readings):
        result = detector.unfold_mlem_stop(
            readings, max_iterations=50, cps_crossover=100.0
        )
    ...
    def test_with_multiple_readings(self, detector):
        ...
        for name in detector.detector_names[:3]:
            readings[name] = 50.0
        result = detector.unfold_mlem_stop(
            readings, max_iterations=100, cps_crossover=100.0
        )
        assert "spectrum" in result
        assert result["method"] == "MLEM-STOP"
        assert len(result["spectrum"]) == detector.n_energy_bins
    ...
    def test_custom_j_threshold(self, detector, readings):
        result = detector.unfold_mlem_stop(
            readings, max_iterations=5, j_threshold=1e10
        )
        assert "spectrum" in result
    ...
    def test_explicit_j_threshold_early_stop(self, detector, readings):
        result = detector.unfold_mlem_stop(
            readings, max_iterations=100, j_threshold=1e10
        )
        assert result.get("iterations", 0) == 1
    ...
    def test_all_readings_same(self, detector):
        ...
        for name in detector.detector_names:
            readings[name] = 10.0
        result = detector.unfold_mlem_stop(
            readings, max_iterations=100, cps_crossover=50.0
        )
        assert "spectrum" in result
