...
class TestRegularization:
    ...
    def test_lcurve_fallback_singular(self, ab):
        ...
        A, b = ab
        with patch.object(
            np.linalg, "solve", side_effect=np.linalg.LinAlgError("sing")
        ):
    ...
    def test_select_reg_parameter_cosine(self, ab):
        ...
        lam = select_regularization_parameter(
            A, b, method="cosine", initial_spectrum=initial
        )
        assert isinstance(lam, float)
    ...
    def test_compare_regularization_methods_with_plot(self, ab, tmp_path):
        ...
        result = compare_regularization_methods(
            A, b, noise_var=0.01, plot=True, plot_path=plot_path
        )
    ...
    def test_randomization_experiment_custom_methods(self, ab):
        ...
        result = randomization_experiment(
            A, b, noise_var=0.01, n_samples=2, methods=["lcurve", "gcv"]
        )
        assert "lcurve" in result
        assert "gcv" in result
...
class TestConverters:
    ...
    def test_convert_sensitivities_to_matrix_with_dict(self):
        ...
        sens = {
            "det1": np.array([1.0, 2.0, 3.0]),
            "det2": np.array([0.1, 0.2, 0.3]),
        }
...
class TestPlotting:
    ...
    def test_plot_with_uncertainty_fill_between(self):
        ...
        fig, ax = plot_with_uncertainty(
            E, spec, uncert_min=uncert_min, uncert_max=uncert_max, show=False
        )
        import matplotlib.pyplot as plt
    ...
    def test_plot_with_uncertainty_errorbar(self):
        ...
        fig, ax = plot_with_uncertainty(
            E, spec, uncert_std=uncert_std, plot_style="errorbar", show=False
        )
        import matplotlib.pyplot as plt
    ...
    def test_plot_comparison_custom_styles(self):
        ...
        fig, ax = plot_comparison(
            results, readings, colors=colors, markers=markers, show=False
        )
        import matplotlib.pyplot as plt
...
class TestUnfoldQpsolversCoverage:
    ...
    def test_qpsolvers_solution_none(self, detector, readings):
        with patch(
            "bssunfold.core.unfold_qpsolvers.solve_qpsolvers", return_value=None
        ):
...
class TestDetectorCoverage:
    ...
    def test_normalize_initial_with_dict(self, detector):
        ...
        det = Detector()
        spectrum_dict = {
            "E_MeV": det.E_MeV.tolist(),
            "Phi": np.ones(det.n_energy_bins).tolist(),
        }
    ...
    def test_plot_with_uncertainty_reference(self, detector, readings):
        ...
        fig, ax = detector.plot_with_uncertainty(
            result, reference_spectrum=ref, show=False
        )
        import matplotlib.pyplot as plt
    ...
    def test_unfold_combined_without_save(self, detector, readings):
        pipeline = [
            {
                "method": "cvxpy",
                "params": {"regularization": 1e-3, "save_result": False},
            },
        ]
...
class TestPlatformCheck:
    ...
    def test_check_qpsolvers_extra_availability_true(self):
        ...
        with patch("bssunfold.platform_check.QPSOLVERS_EXTRA_AVAILABLE", False):
            with patch(
                "bssunfold.platform_check.available_solvers",
                {"osqp", "ecos"},
                create=True,
            ):
...
class TestUnfoldReconstInvert:
    ...
    def test_invert_system_linalg_error_all_retries_fail(self):
        ...
        with patch.object(np.linalg, "cond", return_value=100.0):
            with patch.object(
                np.linalg, "inv", side_effect=np.linalg.LinAlgError("sing")
            ):
...
class TestMaxNeutronEnergy:
    ...
    def test_spectrum_zero_above_emax(self, method):
        ...
        if method == "nnqp":
            kw = dict(max_iterations=500, random_state=0, n_montecarlo=0)
        if method == "qpmad":
            kw = dict(max_iterations=1000, n_montecarlo=0)
